...
def main():
    ...
    print(f'[E4b2] {len(META)} distill pairs (reused)', flush=True)
    ...
    torch.save(net.state_dict(), os.path.join(_ROOT, 'e4b2_unified.pth'))
    ...
    print(f"\n[E4b2 RESULT] head_D {pro['headD']:.2f} ({100*pro['headD']/E4A_REF:.1f}% of E4a {E4A_REF}) | "
          f"recon {pro['recon']:.2f} (E4b control ref 47.79)", flush=True)
    print(f"[E4b2 G1 {'PASS' if g1 else 'FAIL'}] [recon healthy: {'YES' if pro['recon'] >= 46.79 else 'NO'}]", flush=True)
    json.dump({**pro, 'g1': bool(g1)}, open(os.path.join(_ROOT, 'e4b2_result.json'), 'w'), indent=1)
