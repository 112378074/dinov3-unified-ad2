"""E2b — TILE-ALIGNED distillation (walnuts). Fix for E2a-v2's resolution gap (86.4%):
...
BANK_IMGS, STU_IMGS, ITERS, BS, LR = 300, 132, 3000, 12, 2e-4
DEV = 'cuda:0'
...
def main():
    ...
    print('[E2b] bank ready', flush=True)
    ...
    print(f'[E2b] {len(TIL)} tiles  mu={mu:.3f} sd={sd:.3f}', flush=True)
    ...
    print(f'\n[E2b RESULT] teacher {pro_t:.2f} | student(tiled) {pro_s:.2f} | ratio {100*r:.1f}% | corr {c:.4f}', flush=True)
    print(f'[E2b GATE {"PASS" if r >= 0.95 else "FAIL"}]', flush=True)
    json.dump({'teacher_aupro': pro_t, 'student_aupro': pro_s, 'ratio': r, 'corr': c},
              open(os.path.join(_ROOT, 'e2b_result.json'), 'w'), indent=1)
    np.savez_compressed(os.path.join(_ROOT, 'e2b_maps.npz'), tms=tms, sms=sms, gts=gts)
