...
def main():
    ...
    print('[E2d] bank ready', flush=True)
    ...
    print(f'[E2d] {len(TIL)} tiles  mu={mu:.3f} sd={sd:.3f}', flush=True)
    ...
    torch.save(head.state_dict(), os.path.join(_ROOT, 'e2d_head.pth'))   # never lose training again
    ...
    print(f'\n[E2d RESULT] teacher {pro_t:.2f} | student(tiled) {pro_s:.2f} | ratio {100*r:.1f}% | corr {c:.4f}', flush=True)
    print(f'[E2d GATE {"PASS" if r >= 0.95 else "FAIL"}]', flush=True)
    json.dump({'teacher_aupro': pro_t, 'student_aupro': pro_s, 'ratio': r, 'corr': c},
              open(os.path.join(_ROOT, 'e2d_result.json'), 'w'), indent=1)
    np.savez_compressed(os.path.join(_ROOT, 'e2d_maps.npz'), tms=tms, sms=sms, gts=gts)
