...
_CACHE = os.path.join(_ROOT, 'e3b_cache'); os.makedirs(_CACHE, exist_ok=True)
...
def main():
    ...
    bank_n = min(BANK_IMGS, max(60, int(len(tr) * 0.7)))  # bank unchanged
    bank_files = tr[:bank_n]
    stu_files = tr[:min(len(tr), 150)]           # E3b: ALL-train distillation (bank overlap OK — teacher scores anything, no GT involved)
