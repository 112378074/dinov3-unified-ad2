...
_CACHE = os.path.join(_ROOT, 'e3c_cache'); os.makedirs(_CACHE, exist_ok=True)
LAYERS, SCALE, RES, CROP, STRIDE = [7, 15, 23, 31], 0.625, 256, 576, 448
BANK_IMGS, STU_IMGS, N_SYN, ITERS, BS, LR = 300, 150, 80, 6000, 12, 2e-4
DEV = 'cuda:0'
...
def main():
    ...
    for i, f in enumerate(stu_files):
        ...
        pimg = photo(img)                                 # photometric copy: out-of-bank, non-degenerate teacher response
        harvest2 = None
        tms_p = teacher_map_np(pimg); tmt_p = torch.from_numpy(tms_p)[None, None]
        for (x, y) in tiles_of(W, H):
            til = pimg[y:y + CROP, x:x + CROP]
            if til.shape[:2] != (CROP, CROP): continue
            gy = torch.linspace(y * RES / H, (y + CROP - 1) * RES / H, 36) / (RES - 1) * 2 - 1
            gx = torch.linspace(x * RES / W, (x + CROP - 1) * RES / W, 36) / (RES - 1) * 2 - 1
            gr = torch.stack(torch.meshgrid(gy, gx, indexing='ij'), -1)[None][..., [1, 0]]
            TIL.append(til.copy()); TGT.append(F.grid_sample(tmt_p, gr, align_corners=True).squeeze())
