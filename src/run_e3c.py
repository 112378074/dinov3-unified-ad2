...
CACHE = os.path.join(_ROOT, 'e3c_cache')
...
for cat in CATS:
    ...
    with open(os.path.join(LOG, f'e3_{cat}.log'), 'w') as f:
        rc = subprocess.run([PY, '-u', os.path.join(_ROOT, 'src', 'e3c_distill_cat.py'), cat],
...
print(f'[E3c GATE (7cats excl can) {"PASS" if ms >= 0.95*mt else "FAIL"}] (bank-free student MEAN >= 95% teacher)', flush=True)
import json
json.dump({'teacher_aupro': mt, 'student_aupro': ms, 'ratio': ms/max(mt, 1e-9),
          ...
          open(os.path.join(_ROOT, 'e3c_result.json'), 'w'), indent=1)
print(f'=== E3C_DONE {datetime.datetime.now():%m-%d %H:%M} ===', flush=True)
