...
CACHE = os.path.join(_ROOT, 'e3_cache')
...
for cat in CATS:
    ...
    with open(os.path.join(LOG, f'e3_{cat}.log'), 'w') as f:
        rc = subprocess.run([PY, '-u', os.path.join(_ROOT, 'src', 'e3_distill_cat.py'), cat],
...
print(f'[E3a GATE {"PASS" if ms >= 0.95*mt else "FAIL"}] (bank-free student MEAN >= 95% teacher)', flush=True)
import json
json.dump({'teacher_aupro': mt, 'student_aupro': ms, 'ratio': ms/max(mt, 1e-9),
          ...
          open(os.path.join(_ROOT, 'e3a_result.json'), 'w'), indent=1)
print(f'=== E3A_DONE {datetime.datetime.now():%m-%d %H:%M} ===', flush=True)
